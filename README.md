# 🛍️ PriceComp - AI-Powered Price Comparison Engine
...
PriceComp uses a **Proxy Server Architecture** to overcome CORS restrictions and handle heavy compute tasks.

```mermaid
User Request (Frontend) 
   │
   ▼
Node.js Server (Backend) ──┬──> [Check Cache] (Hit? Return Data)
                           │
                           ├──> [Amazon API Request]
                           ├──> [Flipkart Stealth Scraper]
                           ├──> [Snapdeal HTML Parser]
                           │
                           ▼
                 [Merge & Sort Data]
                           │
